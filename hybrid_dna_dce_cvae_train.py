...
# Task-0 VAE scaling and optional decoder freeze to prioritize classifier learning.
TASK0_BETA_VAE = 0.01        # effective beta_vae to use for task 0
TASK0_FREEZE_DECODER_EPOCHS = 3  # if >0, freeze decoder for these many epochs of task 0
...
def add_task_exemplars_from_loader(exemplar_memory: ExemplarMemory, data_loader, version_id: int, max_per_class: int = None):
    """
    Add exemplars once per task, after the task finishes.
    max_per_class: if set, limit the number of exemplars added per class.
    This avoids contaminating the current task's training with exemplars from the same task.
    """
    counts = {}
    for batch in data_loader:
        # support DataLoader that yields tensors (x,y) or lists
        ...
        B = x_batch.size(0)
        for i in range(B):
            cls = int(y_batch[i].item())
            if max_per_class is not None and counts.get(cls, 0) >= max_per_class:
                continue
            try:
                exemplar_memory.add_batch_with_version(x_batch[i:i+1], y_batch[i:i+1], version_id)
                counts[cls] = counts.get(cls, 0) + 1
            except Exception:
                # fall back to add_batch if version API not present
                try:
                    exemplar_memory.add_batch(x_batch[i:i+1], y_batch[i:i+1])
                except Exception:
                    pass

def _maybe_freeze_decoder_for_task0(model: DCECVAEModule, epoch: int):
    """
    Freeze decoder (if present) for first TASK0_FREEZE_DECODER_EPOCHS epochs
    when training task 0, to prioritize classifier learning.
    """
    if TASK0_FREEZE_DECODER_EPOCHS <= 0:
        return  # feature disabled
    if epoch < TASK0_FREEZE_DECODER_EPOCHS:
        # try common decoder attribute names
        for name in ("decoder", "decoder_img", "decoder_cnn", "decoder_fc"):
            if hasattr(model, name):
                module = getattr(model, name)
                for p in module.parameters():
                    p.requires_grad = False
...
def train_one_task_hybrid(
    ...
    for epoch in range(epochs_per_task):
        # optional decoder freeze for task 0 warmup
        if task_id == 0:
            _maybe_freeze_decoder_for_task0(model, epoch)
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            # only use exemplars if there are previous tasks (task_id>0)
            # -----------------------------------------------------------------
            z_ex, y_ex = None, None
            if task_id > 0 and exemplar_local is not None and exemplar_local.total_count() > 0:
                cur_ex_count = max(1, int(batch_size * 0.45))
                x_ex, y_ex, v_ex = exemplar_local.sample_balanced(cur_ex_count, device, return_versions=True)
                if x_ex is not None:
                    try:
                        z_ex = model.encode_to_mu(x_ex, y_ex)
                        z_ex = _project_batch_by_versions(adapters, z_ex, v_ex)
                    except Exception:
                        z_ex = None
            ...
            # choose effective beta_vae (weaker on first task)
            beta_vae_effective = TASK0_BETA_VAE if task_id == 0 else beta_vae
            ...
            loss = ce_loss + beta_vae_effective * vae_loss + 0.5 * base_align * L_align + stability_gamma * L_stability
            ...
            # Update hot buffer after step (store CPU)
            with torch.no_grad():
                try:
...
def hybrid_dna_dce_cvae_continual_learning(
    ...
    latent_dim: int = 128,
    lr: float = 5e-4,
    beta_vae: float = 0.2,                 # default VAE weight (task > 0)
    epochs_per_task: int = 8,
    ...
    # ensure exemplar memory cleared at start
    try:
        exemplar_memory.clear()
    except Exception:
        pass
    ...
    for task_id, (train_loader, test_loader, labels) in enumerate(tasks):
        ...
        try:
            ...
            # fallback: include nothing in enc_params if attributes missing
            enc_params = []
        ...
        # Train the task (note: this function will update hot_buffer but will NOT add exemplars per-batch)
        ...
        # After finishing training for this task: snapshot exemplars (one-time)
        try:
            add_task_exemplars_from_loader(exemplar_memory, train_loader, current_version, max_per_class=exemplar_per_class)
        except Exception:
            pass
